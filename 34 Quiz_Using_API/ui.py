...
class QuizInterface:
    ...
    def __init__(self, quiz_brain: QuizBrain):
        ...
        self.question_text = self.canvas.create_text(150, 125, width=280, font=(FONT_NAME, 20, "italic"))
        self.canvas.grid(row=1, column=0, columnspan=2, padx=20, pady=20)

        true_image = PhotoImage(file="images/true.png")
        false_image = PhotoImage(file="images/false.png")

        self.true_button = Button(image=true_image, command=self.true_action)
        self.true_button.grid(row=2, column=0, pady=20)

        self.false_button = Button(image=false_image, command=self.false_action)
